...
/** The single-tile stand-in for the step cards - see `HOW_IT_WORKS_DETAILS`. */
function DetailTile({ detail }: { detail: HIW_DETAIL }) {
  return (
    <div className="w-full rounded-xl border bg-white p-6 sm:p-8">
      <div className="flex flex-col gap-4">
        <div className="flex items-start gap-4">
          <div className="flex size-12 shrink-0 items-center justify-center rounded-2xl border-2 bg-primary/10 border-primary/20 text-primary">
        ...
        <p className="text-sm text-gray-500 sm:text-base">{detail.desc}</p>
        <ul className="grid grid-cols-1 gap-2 text-sm text-gray-500 sm:grid-cols-2 sm:gap-x-6">
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex w-full flex-col items-center gap-8">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            {/* <p className="max-w-2xl text-sm text-gray-500 sm:text-base">
              ...
              {isAuctionFeatureActive
                ? ' Buy your way: bid on a live auction, or pay a fixed price on a buy now listing.'
                : ' Every lot carries one fixed price — pay it and the stock is yours.'}
            </p> */}
          </div>
          ...
          <Tabs defaultValue={HOW_IT_WORKS_STEPS[0]?.id} className="w-full items-center">
            ...
            {HOW_IT_WORKS_STEPS.map((step) => {
              const detail = HOW_IT_WORKS_DETAILS.find((el) => el.id === step.id)
              return (
                <TabsContent
                  key={step.id}
                  value={step.id}
          ...
          {/* one purchase mode needs no comparison - a lone card under a "two
              ways to buy" heading would read as a bug */}
          {isAuctionFeatureActive && (
            <div className="flex w-full flex-col items-center gap-4">
              ...
              <p className="max-w-2xl text-center text-sm text-gray-500">
                Both modes use the same one-off security deposit, so you're
                free to bid on one lot and buy another outright.
              </p>
            </div>
          )}
