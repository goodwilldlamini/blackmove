...
export function AuctionDetailsTopBar({
  ...
  return (
    <div className="sticky top-0 z-10 mx-auto grid w-full max-w-6xl grid-cols-1 gap-4 bg-white px-2 py-4 sm:grid-cols-5 sm:bg-transparent sm:px-0">
      <div className="flex items-center gap-2 sm:col-span-3 sm:gap-4">
        <div className="flex flex-1 items-center justify-between gap-3 rounded-2xl bg-gray-700 px-3 py-3 shadow-sm divide-x divide-white/20 sm:px-6 sm:py-4">
          {isBuyNow ? (
            <>
              <Stat label="price">
...
function Stat({
  ...
  return (
    <div className={`flex min-w-0 flex-1 flex-col items-center gap-1 px-2 sm:items-start sm:px-0 ${className || ''}`}>
      <span className="hidden text-[10px] font-semibold tracking-wide text-gray-400 uppercase sm:block">
        {label}
      </span>
      {children}
    </div>
